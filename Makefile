...
.PHONY: build build-full build-public build-both view view-full view-public clean watch install check build-image push-image
...
view-full:
	evince $(OUTPUT_PDF_FULL) 2>/dev/null &

view-public:
	evince $(OUTPUT_PDF_PUBLIC) 2>/dev/null &
...
check:
	...
	@chktex $(MAIN_FILE_FULL)
	@chktex $(MAIN_FILE_PUBLIC)
